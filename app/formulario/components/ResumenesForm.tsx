...
import { FiSave, FiTrash2, FiEdit2, FiCalendar, FiFileText, FiTag, FiType, FiList, FiLink, FiImage, FiX, FiBold, FiItalic, FiUnderline, FiAlignLeft, FiAlignCenter, FiAlignRight } from 'react-icons/fi';
...
const SimpleEditor = ({ value, onChange, placeholder }) => {
  ...
  const restoreSelection = useCallback(() => {
    ...
    try {
      ...
      range.setStart(lastSelectionRef.current.startContainer, lastSelectionRef.current.startOffset);
      range.setEnd(lastSelectionRef.current.endContainer, lastSelectionRef.current.endOffset);
  ...
  const updateFormatState = useCallback(() => {
    ...
    setFormatState({
      ...
      justifyLeft: document.queryCommandValue('justify') === 'left' || 
...
export default function ResumenesForm() {
  ...
  const fetchData = async () => {
    try {
      const [resumenesRes, boletinesRes, categoriasRes] = await Promise.all([
  ...
  const onSubmit = async (data) => {
    if (!data.contenido || data.contenido.trim() === '' || data.contenido === '<br>') {
            ...
            >
              ...
              {boletines.map((boletin) => (
                <option key={boletin.id} value={boletin.id}>
                  {boletin.tipo_boletin} - {new Date(boletin.fecha).toLocaleDateString()}
                </option>
              ))}
            </select>
            {errors.id_boletin && (
              <p className="mt-1 text-sm text-red-600">{errors.id_boletin.message}</p>
            )}
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              {...register('contenido', { 
                ...
                validate: value => value && value.trim() !== '' && value !== '<br>' || 'Escribe contenido'
              })}
            />
        ...
        {resumenes.length === 0 ? (
          ...
          <div className="space-y-4">
            {resumenes.map((resumen) => (
              <div key={resumen.id} className="border rounded-lg p-4 hover:bg-gray-50">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-medium">
                      {categorias.find(c => c.id === resumen.id_categoria)?.nombre || 'Sin categoría'}
                    </p>
                    ...
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ 
                        __html: resumen.contenido?.replace(/<[^>]+>/g, ' ').substring(0, 100) + '...'
                      }}
                    />
                  </div>
